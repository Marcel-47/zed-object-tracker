...
def _pos_label(target, unit: str) -> str:
    x_dir = "right" if target.x >= 0 else "left"
    y_dir = "up"    if target.y >= 0 else "down"
    z_dir = "away"  if target.z >= 0 else "towards"
    return f"X:{target.x:+.2f}{unit} ({x_dir})  Y:{target.y:+.2f}{unit} ({y_dir})  Z:{target.z:+.2f}{unit} ({z_dir})"


def draw(frame, all_targets, id_to_num: dict, selected_num: int | None, fps=None, input_buffer: str = "", show_overlay: bool = True, auto_select: bool = False, color_filter: str = "", unit: str = "m", unit_per_meter: float = 1.0):
    ...
    if not all_targets:
        ...
        for target in all_targets:
            ...
            if show_overlay:
                ...
                text_y = max(y1 - 8, 15)
                cv2.putText(out, _pos_label(target, unit), (x1, text_y), FONT, 0.45, color, 1)
    ...
    # _MAP_*_RANGE are in meters; convert to the active unit so the map covers
    # the same physical area regardless of coordinate_units.
    x_range = _MAP_X_RANGE * unit_per_meter
    z_range = _MAP_Z_RANGE * unit_per_meter
    for target in all_targets:
        num = id_to_num.get(target.track_id)
        is_selected = num is not None and num == selected_num
        dot_color = RED if is_selected else GREEN
        dot_x = mx + _MAP_W // 2 + int((target.x / x_range) * (_MAP_W // 2))
        dot_y = my + _MAP_H - 6 - int((target.z / z_range) * (_MAP_H - 12))
