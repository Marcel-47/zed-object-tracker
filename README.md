...
- HUD: live FPS counter, object count, auto-select indicator, active color filter, top-down position map (bottom-left corner)
...
| Key | Action |
|-----|--------|
| `0`–`9` | Type a display number to select an object (type `0` to deselect) |
| `Enter` | Confirm the typed number (selecting `0` clears the selection) |
...
| `f` | Cycle the color filter (off → blue → red → green → yellow → orange → purple) |
| `h` | Toggle overlay visibility (hides text labels, keeps bounding boxes) |
| `r` | Reset everything (numbering, selection, overlay, auto-select, color filter) |
| `q` | Quit |
